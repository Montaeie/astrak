...
export function CTABlockRenderer({ block }: CTABlockRendererProps) {
  const title = block.title || 'Prêt à propulser ta visibilité sur Google?'
  const subtitle = block.subtitle || 'Commence par un audit SEO gratuit et découvre ton potentiel de croissance'
  const buttonText = block.buttonText || 'Prendre Rendez-vous'
